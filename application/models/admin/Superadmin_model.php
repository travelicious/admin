...
/*__________________________ notification query ____________________ */
    ...
    function fetch_employee_detail($id) {
        $query = $this->db->query("select tbl_user.*, customer.name,customer.id as cst_id,customer.email as cst_email  from tbl_user, customer where tbl_user.id = $id and customer.assign_to = $id ");
        return $query;
    }
